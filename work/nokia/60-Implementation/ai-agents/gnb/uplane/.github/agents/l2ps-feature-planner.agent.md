...
- **Knowledge hierarchy:** before designing, read in order (only as needed):
  ...
  4. **`L2PS-coding`** agent (VS Code / GitHub Copilot auto-loads from `/workspace/.github/agents/L2PS-coding.md`) — only the sections relevant to the affected components, for defaults you may explicitly re-state in the Blueprint section.
