...
- **Knowledge hierarchy:** read these as needed:
  ...
  2. **`L2PS-coding`** agent (`/workspace/.github/agents/L2PS-coding.md`, coding standards), **`L2PS-ut`** agent (`/workspace/.github/agents/L2PS-ut.md`, UT patterns), and **`local-smith`** agent (`/workspace/.github/agents/local-smith.agent.md`, robustness ruleset router and per-category rule mapping) — all VS Code / GitHub Copilot auto-loads.
  3. The relevant rule files under `/workspace/.github/rules/review/uplane/` (only those applicable to the file category).
...
- **Read** the applicable rule files under `/workspace/.github/rules/review/uplane/` for each file category encountered (see <a href="#classification">File classification</a>).
...
| Ruleset (file under `/workspace/.github/rules/review/uplane/`) | L2-PS Prod | L2-PS UT | FUSE |
|----------------------------------------------------------------|:----------:|:--------:|:----:|
| `index.rules.md`                       | x |   |   |
...
When the pipeline runtime supports subagent invocation, prefer delegating the per-ruleset evaluation to `@local-smith`. Use the prompt template from the `local-smith` agent (`/workspace/.github/agents/local-smith.agent.md`), the "Subagent Prompt Template" section, passing only the files belonging to the relevant category. Aggregate the findings.
