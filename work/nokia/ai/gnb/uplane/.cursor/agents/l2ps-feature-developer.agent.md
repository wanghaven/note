...
- **Knowledge hierarchy:** before coding, read in order:
  ...
  3. **`L2PS-coding`** agent (`/workspace/.cursor/agents/L2PS-coding.md`, Cursor auto-loads) — coding standards, mandatory.
  4. **`i-faster`** skill (Cursor auto-loads) — build CLI.
...
The authoritative L2-PS coding rules live in the `L2PS-coding` agent (`/workspace/.cursor/agents/L2PS-coding.md`, Cursor auto-loads); read it when you touch a new component or are unsure. The non-negotiable invariants the Reviewer will check on every diff, kept here for fast reference:
...
Use `i_faster` to compile L2-PS. The full command grammar (help / dry-run / target variants / pre-flight artifact paths) lives in the `i-faster` skill (Cursor auto-loads); read it once and then use the canonical commands below.
