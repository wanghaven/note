...
- **Knowledge hierarchy:** read these as needed:
  ...
  3. **`l2ps-fuse-sct`** agent (`/workspace/.cursor/agents/l2ps-fuse-sct.agent.md`, Cursor auto-loads) — primary how-to for build/run/debug.
...
The L2-PS feature pipeline calls this agent up to **3 cycles per FP** and runs several FPs in series. A full FUSE-SCT compile + run is the largest single cost in the pipeline; blanket `i_faster bfsct` / `i_faster rfsct` invocations dominate runtime without adding information. This agent therefore enforces a **focused-first** policy. The `i_faster` commands themselves are documented in the `i-faster` skill (Cursor auto-loads; read it once for syntax, target variants, debug helpers, log inspection); the policy and gate rules are owned here.
